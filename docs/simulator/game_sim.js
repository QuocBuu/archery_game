...
function findWasmBinary() {
  return locateFile('game_sim.wasm');
}
...
var ASM_CONSTS = {
 ...
 159094: ($0, $1, $2, $3, $4) => { var w = $0; var h = $1; var hot_x = $2; var hot_y = $3; var pixels = $4; var canvas = document.createElement("canvas"); canvas.width = w; canvas.height = h; var ctx = canvas.getContext("2d"); var image = ctx.createImageData(w, h); var data = image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = (val >> 24) & 0xff; src++; dst += 4; } } else { var data32 = new Int32Array(data.buffer); num = data32.length; data32.set(HEAP32.subarray(src, src + num)); } ctx.putImageData(image, 0, 0); var url = hot_x === 0 && hot_y === 0 ? "url(" + canvas.toDataURL() + "), auto" : "url(" + canvas.toDataURL() + ") " + hot_x + " " + hot_y + ", auto"; var urlBuf = _malloc(url.length + 1); stringToUTF8(url, urlBuf, url.length + 1); return urlBuf; },  
 160082: ($0) => { if (Module['canvas']) { Module['canvas'].style['cursor'] = UTF8ToString($0); } },  
 160165: () => { if (Module['canvas']) { Module['canvas'].style['cursor'] = 'none'; } },  
 160234: () => { return window.innerWidth; },  
 160264: () => { return window.innerHeight; }
};

// Imports from the Wasm binary.
var _malloc = makeInvalidEarlyAccess('_malloc');
var _free = makeInvalidEarlyAccess('_free');
var _sim_web_button_set = Module['_sim_web_button_set'] = makeInvalidEarlyAccess('_sim_web_button_set');
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports['sim_web_button_set'] != 'undefined', 'missing Wasm export: sim_web_button_set');
  ...
  _sim_web_button_set = Module['_sim_web_button_set'] = createExportWrapper('sim_web_button_set', 2);
